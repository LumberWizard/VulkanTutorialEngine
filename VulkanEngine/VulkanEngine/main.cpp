...
struct QueueFamilyIndices {
	...
	bool mixedGraphicsPresentQueue() {
		return graphicsFamily.has_value() && presentFamily.has_value() && graphicsFamily.value() == presentFamily.value();
	}

	bool separateGraphicsTransferQueues() {
		return graphicsFamily.has_value() && transferFamily.has_value() && transferFamily.value() != graphicsFamily.value();
	}

	bool familiesOptimized() {
		return mixedGraphicsPresentQueue() && separateGraphicsTransferQueues();
	}
};
...
private:
	...
	int rateDeviceSuitability(VkPhysicalDevice device) {
		...
		// Application can't function without geometry shaders or a graphics supporting queue family
	...
	void createCommandPools() {
		...
		if (queueFamilyIndices.separateGraphicsTransferQueues()) {
			transferPoolInfo.sType = VK_STRUCTURE_TYPE_COMMAND_POOL_CREATE_INFO;
			transferPoolInfo.queueFamilyIndex = queueFamilyIndices.transferFamily.value();
		}

		if (vkCreateCommandPool(device, &graphicsPoolInfo, nullptr, &graphicsCommandPool) != VK_SUCCESS || 
			(queueFamilyIndices.separateGraphicsTransferQueues() && (vkCreateCommandPool(device, &transferPoolInfo, nullptr, &transferCommandPool) != VK_SUCCESS))) {
